...
// ─── Estados reutilizados de barberia/page.tsx ────────────────────────────────
...
export default function PersonalizarPage() {
  ...
  // Sincronizar descripción solo en la primera carga de la barbería
  useEffect(() => {
    if (barbershop) {
  ...
  // Redirigir si no es owner (solo cuando la carga terminó)
  useEffect(() => {
  ...
  const busy = uploading || saving || savingDesc;

  // ─── Logo ─────────────────────────────────────────────────────────────────

  const handleLogoChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    // Limpiar el input antes de cualquier await para evitar re-disparos
    ...
    try {
      ...
      // refresh() re-carga barbershop desde la DB sin reiniciar loading
      await refresh();
    } catch {
  ...
  const handleSaveDescription = async () => {
    setSavingDesc(true);
    try {
      // updateBarbershop de useBarbershop actualiza DB y estado local simultáneamente
  ...
  // null mientras el useEffect de redirect corre (evita flash de contenido)
  if (!barbershop || !isOwner) return null;

  const logoInitial = barbershop.name.charAt(0).toUpperCase();
    ...
    <div className="space-y-6 p-6 lg:p-8">
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-3xl font-bold text-gray-900">Personalizar barbería</h1>
